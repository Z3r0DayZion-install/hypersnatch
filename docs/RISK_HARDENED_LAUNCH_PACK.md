...
HyperSnatch is a deterministic, offline-first static analysis utility that parses raw HTML payloads to surface structural DOM variables and endpoint manifests.
...
*   **Integrity Reporting:** Generates a verifiable `.manifest.json` detailing surfaced endpoints.
...
**3) 10-Word Tagline:**
Deterministic static analysis for structural HTML variable and endpoint mapping.

**4) Forbidden Words List + Safe Replacements:**
| 🚫 Forbidden Word / Phrase | ✅ Safe Replacement |
| :--- | :--- |
| Download / Downloader | Analyze / Analyzer |
| Video / Stream URL | Payload Endpoint / Manifest Vector |
...
A: It performs deterministic static analysis on raw HTML source code to trace obfuscated DOM variables and surface underlying endpoint manifests.
...
A: Never. The analysis engine is 100% offline. The only network request made is a periodic cryptographic check of your license key.
...
*   *Trigger:* Engine completes parsing but returns 0 high-confidence vectors AND the raw HTML length is under 15KB with `<script>` tags forming the bulk of the body.
*   *UI Copy:* `Analysis Complete: 0 Endpoints Surfaced. [View SPA Formatting Guide]`
...
*   *Trigger:* A user emails a screenshot of a red Microsoft Defender or Chrome "Dangerous Download" block.
...
**Day 6: Second Niche Release (r/DataHoarder)**
*   Post the strict "no-cloud dependency" draft.
...
*   *Decision Tree:*
    ...
    *   If refund rate > 15% -> The tool's heuristics are failing on standard targets. Delay HN by 1 week to write patches.
