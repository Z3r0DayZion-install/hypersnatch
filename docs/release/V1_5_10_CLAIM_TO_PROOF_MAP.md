...
| Claim ID | Claim | Current Proof Type | Direct vs Indirect | Primary Evidence Surface | Conservative Allowed Wording | Upgrade Path (if indirect) | Slice 4 Status |
|---|---|---|---|---|---|---|---|
...
| C-11 | Binary trust is externally accepted/signed | Conditional signing evidence | Indirect/conditional | signing artifacts + release note evidence when signing is enabled | "signed trust claim only when signing evidence is explicitly recorded" | define/enforce signing contract in strict release policy | Bounded-Deferred (PDG-02) |
