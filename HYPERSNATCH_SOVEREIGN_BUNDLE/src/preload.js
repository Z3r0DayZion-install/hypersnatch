...
contextBridge.exposeInMainWorld('vaultSearch', {
  ...
  finalFreeze: (caseData, reports) => ipcRenderer.invoke('final-freeze', { caseData, reports }),
  getHardwareStatus: () => ipcRenderer.invoke('get-hardware-status'),
  authenticateLicense: (path) => ipcRenderer.invoke('authenticate-license', path),
  empireSync: (action, data) => ipcRenderer.invoke('empire-sync', { action, data }),
